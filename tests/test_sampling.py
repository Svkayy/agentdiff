...
def test_run_samples_timeout_fails_with_message_and_retries(tmp_path, monkeypatch, capsys):
    ...
    # Exact match: the persisted error must be the original message with no
    # "RuntimeError: " (or any other) prefix from the exception-format path.
    assert ts.trajectories[0].error == "sample timed out after 0.1s"
...
def test_run_samples_strict_redaction_reaches_pool_worker_thread(tmp_path, monkeypatch):
    """Important 1 (round 2) regression test.

    With ``workers >= 2``, ``run_samples`` submits each sample to a
    ``ThreadPoolExecutor`` pool via ``executor.submit(_run_one_sample_with_retry, ...)``.
    Pool worker threads do not inherit the submitting (main) thread's
    ContextVar context, so ``set_active_redaction_config(strict)`` -- set on
    the main thread before calling ``run_samples`` -- is invisible inside the
    pool worker thread. The inner ``copy_context()`` done later in
    ``_run_traced_with_timeout`` only re-captures whatever (wrong, default)
    context the pool worker thread already has; it cannot recover context lost
    at the pool-submission boundary.

    This must fail before the fix (capturing ``contextvars.copy_context()`` on
    the main thread immediately before each ``executor.submit`` call, and
    running the submitted work via ``ctx.run(...)``) and pass after.
    """
    import agentdiff

    (tmp_path / "httprunner2.py").write_text(
        "import httpx\n"
        "def run(input):\n"
        "    client = httpx.Client()\n"
        "    client.post(\n"
    ...
    try:
        ...
        with respx.mock() as rmock:
            ...
            written = run_samples(
                ...
                structure_root=tmp_path,
                progress=False,
                # timeout_seconds > 0 (default) + workers=2 forces the
                # pool-submit path that must propagate context.
                timeout_seconds=5.0,
                workers=2,
            )
