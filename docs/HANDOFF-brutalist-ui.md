...
- **T3 (marketing) is now COMPLETE too** — its final work is folded into `aa2435b`/`b00b2b5` (shared working tree; no separate `feat(marketing):` commit — nothing was left to commit). All USER REQUIREMENTS applied per its report: **pricing section fully removed** (component deleted, nav link + `#pricing` anchor gone, indices renumbered RAW_DATA 001 / METHODOLOGY 002 / ECOSYSTEM 003), **real before/after agent-topology graph** in About (orchestrator → retriever/fact_checker/summarizer, CANDIDATE `fact_checker` STOPPED solid `#ea580c`), **OSS framing + Codebase Map kept**, legibility fixed (micro-labels → 11px, body → text-sm/base, light muted contrast ≈4.5:1 AA). **NOT independently reviewed.**
- **NET: both T3 and T4 restyle work is DONE and committed; branch tip builds clean (build + build:cli + 41 tests). Remaining = (a) independently review both restyles, (b) fix the CLI RigorBanners bug below, (c) T5.**
- **T5 cleanup note from T3:** dead legacy marketing files (old `Nav/Hero/Features/...` under `frontend/src/pages/marketing/`) were left in place for T5 retirement; also `security@agentdiff.dev` is a placeholder inbox.
